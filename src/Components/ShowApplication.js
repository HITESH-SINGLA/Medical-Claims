...
function ShowApplication() {
  ...
  const getData = async () => {
    const res = await fetch("http://172.30.2.244:5003/showApplicationId/" + id, {
      method: "POST",
  ...
  const getStatus = async () => {
    const res2 = await fetch(
      "http://172.30.2.244:5003/showApplicationIdStatus/" + id,
      {
        method: "POST",
        body: JSON.stringify({ user_data }),
        headers: { "Content-Type": "application/json" },
  ...
  const resubmit_application = async (e) => {
    ...
    if (
      ...
      const res4 = await fetch("http://172.30.2.244:5003/resubmitApplication", {
        method: "POST",
        body: JSON.stringify({
          ...new_obj,
